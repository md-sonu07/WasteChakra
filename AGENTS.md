...
- **Centralized icon system (`src/components/AppIcons.jsx`)** — project-wide migration from inline `material-symbols-outlined` spans to a single `<Icon name="...">` component (~296 refs across 60 files; imports added per file using `./AppIcons`, `../components/AppIcons`, or `../../components/AppIcons` paths).
  - `ICON_MAP` = 152 snake_case Material names → named lucide-react imports (tree-shaken, NOT `import * as`). All imports verified to exist in `lucide-react` 1.41.0. During later audits added: `card_giftcard→Gift`, `delete_forever→Trash2`, `notifications_off→BellOff`, `place→MapPin`, `speed→Gauge`, `timer→Timer`, `today→CalendarDays`, `vertical_align_bottom→ArrowDownToLine`, `local_activity→Activity`, `percent→Percent`.
...
- **Backend E2E verified via curl** (SQLite override, port 8000): register, login, waste-report create, pickup create, admin assign, state transitions, weight/collected_at capture, invalid-transition rejection, passport auto-creation, citizen `/user/impact/` returns totals. Demo users confirmed (password `admin12345`). Note: pickup create response omits `id`/`pickup_id` — resolve via list endpoint.
